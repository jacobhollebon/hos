...
class HOSObjectEncoderController( visr.AtomicComponent ):
    ...
    def __init__( self,
        ...
        self.HOSAngles = hos.toolboxes.calculator.calculateHOSAngle(self.objectPos_xyz, self.hhat) # Source angles w.r.t listener frame
        srcEncoder = hos.toolboxes.calculator.calculateHOSPlant(self.HOSAngles, self.HOSOrder, HOSType=self.HOSType) # encoder
...
#                        print("\r" + "ch " + str(ch), end="")
        ...
        if recalculateFlag:
            # Calculate Plant Matrix (encoding coefficents for each source from each given direction)
            self.HOSAngles = hos.toolboxes.calculator.calculateHOSAngle(self.objectPos_xyz, self.hhat) # Source angles w.r.t interaural axis
            srcEncoder = hos.toolboxes.calculator.calculateHOSPlant(self.HOSAngles, self.HOSOrder, HOSType=self.HOSType) # encoder
            self.srcEncoder = srcEncoder * self.levels[np.newaxis,:]
