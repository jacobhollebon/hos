...
def hoa2hosDecoder(hoa_nm, kind="realsn3d"):
    ...
    if dims > 3:
        ...
        raise ValueError(
            "Data input should have at minimum 2 dimensions, coefficients x (time/freq)"
        )
...
def hoa2hosBinauralDecoder(
    ...
    if hoa_hrir_nm is not None:
        ...
        if not np.sum(np.imag(hoa_hrir_nm)):
            warnings.warn(
                "You have supplied a complex valued hoa_hrir_nm, this should be time domain data... performing an irfft"
            )
    ...
    binaural = decoders.hos2BinauralDecoder(
        hos_nm,
