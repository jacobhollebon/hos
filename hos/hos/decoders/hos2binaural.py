...
def hos2BinauralDecoder( hos_nm, hos_hrir_nm=None, sofaPath=None, kind='realsn3d', beta=1e-15, usemagls=False, maglsArgs={}):
    '''
    ...
    if hos_hrir_nm is not None:
        ...
        if not np.sum(np.imag(hos_hrir_nm)):
            warnings.warn('You have supplied a complex valued hos_hrir_nm, this should be time domain data... performing an irfft')
            hos_hrir_nm = np.fft.irfft(hos_hrir_nm, axis=-1)
            
    # Generate the hrir coefficients
    if hos_hrir_nm is None:
        if usemagls:
            hoa_hrir_nm = spherical.sofa2shmagls( sofaPath, N=N, kind=kind, beta=beta, **maglsArgs)
        else:
